/* global Promise, labelPriorities, productLabelsEnable */
import { getModuleOptions, handleize, ready } from 'lib/dom'
...
const getLabelOrders = (data, isMultiple = false) => {
  const tagLabelsFormat = data
  const tagLabelOrders = []

  if (labelPriorities.length > 0 && tagLabelsFormat.length > 0) {
    for (const itemPriority of labelPriorities) {
      const isExits = tagLabelsFormat.some((item) => handleize(item.replace('label-', '')) === itemPriority.key)
      if (isExits) {
        tagLabelOrders.push(itemPriority)
      }
    }
  }

  tagLabelsFormat.map((tag) => {
    const isExits = tagLabelOrders.some((item) => handleize(tag.replace('label-', '')) === item.key)
    if (!isExits) {
      const nameTag = tag.replace('label-', '')
      const keyTag = handleize(nameTag)
      tagLabelOrders.push({ key: keyTag, name: nameTag })
    }
  })

  if (tagLabelOrders.length === 0) {
    return tagLabelOrders
  }
  return isMultiple ? tagLabelOrders : [tagLabelOrders[0]]
}

const showProductLabel = productLabelsEnable || false
